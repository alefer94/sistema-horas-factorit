...
if ($ajaxUsrID === -1){
	...
	try {
		$uslID = PDOSkillUsuarios::insert(
			new InterfazPDO(),
			$_POST
		);
		if ($uslID != 0) {
			$callback_success = true;
			$callback_response = "Skill-Nivel Creado.";
		}
		else {
			$callback_response = "El skill nivel no pudo ser agregado. Verifique que su código y/o nombre no se repitan e inténtelo nuevamente.";
		}
	}
