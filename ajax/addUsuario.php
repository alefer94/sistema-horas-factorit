 ...
 * Cliente: FactorIT
 * Archivo fuente: ajax/addUsuario.php
 * Autor: Benjamin La Madrid
 * 
 * Controlador AJAX para crear un usuario. 
*/
...
if ($ajaxUsrID === -1){
	...
	if (!empty($_POST["usrRut"])) { $_POST["usrClave"] = $_POST["usrRut"]; }
	 
	try {
		$usrID = PDOUsuarios::insert(
			new InterfazPDO(), 
			$_POST
		);
		if ($usrID !== 0) {
			$callback_success = true;
			$callback_response = "Usuario creado.";
		}
		else {
			$callback_response = "El usuario no pudo ser agregado.<br/>Verifique que su e-mail, rut y/o nombre no se repitan e inténtelo nuevamente.";
		}
	}
