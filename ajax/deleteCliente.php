...
if ($ajaxUsrID === -1){
	...
	if (isset($_POST["cltID"])){
		...
		try {
			$pdo = new InterfazPDO();
			$filas_afectadas = PDOClientes::remove( $pdo, intval($_POST["cltID"]) );
			unset($pdo);
			if ($filas_afectadas != 0) {
				$callback_success = true;
				$callback_response = "Cliente eliminado.";
			}
			else {
				$callback_response = "El cliente no pudo ser eliminado.<br/>Por favor, inténtelo nuevamente.";
			}
		}
		...
		$callback_response = "No se recibió el parámetro 'cltID' para eliminar un cliente.";
	}
}
