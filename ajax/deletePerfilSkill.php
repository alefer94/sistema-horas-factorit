...
if ($ajaxUsrID === -1){
	...
	if (isset($_POST["uslID"])){
		...
		try {
			$pdo = new InterfazPDO();
			$filas_afectadas = PDOSkillUsuarios::remove( $pdo, intval($_POST["uslID"]) );
			unset($pdo);
			if ($filas_afectadas != 0) {
				$callback_success = true;
				$callback_response = "Skill-Nivel eliminado.";
			}
			else {
				$callback_response = "Skill-Nivel no pudo ser eliminado.<br/>Por favor, recargue la página e inténtelo nuevamente.";
			}
		}
