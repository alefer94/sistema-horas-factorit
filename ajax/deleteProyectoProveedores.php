...
if ($ajaxUsrID === -1){
	...
	if (isset($_POST["prjProID"])){
		...
		try {
			$pdo = new InterfazPDO();
			$filas_afectadas = PDOProyectoUsuario::remove_proveedores( $pdo, intval($_POST["prjProID"]) );
			unset($pdo);
			if ($filas_afectadas != 0) {
				$callback_success = true;
				$callback_response = "colaborador externo eliminado.";
			}
			else {
		...
		$callback_response = "No se recibió el parámetro 'prjProID' para eliminar un registro de horas.";
	}
}
