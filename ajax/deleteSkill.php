 ...
 * Cliente: FactorIT
 * Archivo fuente: ajax/deleteSkill.php
 * Autor: Diego Garcia
 * 
 * Controlador AJAX para eliminar un skill. 
*/
...
if ($ajaxUsrID === -1){
	...
	if (isset($_POST["sklID"])){
		...
		try {
			$pdo = new InterfazPDO();
			$filas_afectadas = PDOSkill::remove( $pdo, intval($_POST["sklID"]) );
			unset($pdo);
			if ($filas_afectadas != 0) {
				$callback_success = true;
				$callback_response = "Skill eliminado.";
			}
			else {
				$callback_response = "La skill no pudo ser eliminado.<br/>Por favor, inténtelo nuevamente.";
			}
		}
		...
		$callback_response = "No se recibió el parámetro 'sklID' para eliminar un skill.";
	}
}
