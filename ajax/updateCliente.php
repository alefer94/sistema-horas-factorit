...
if ($ajaxUsrID === -1){
	...
	if (isset($_POST["cltID"]) && is_numeric($_POST["cltID"])){
		if (sizeof($_POST) > 1) {
			...
			try {
				$pdo = new InterfazPDO();
				$filas_afectadas = PDOClientes::update(
					$pdo, 
					$_POST, 
				  [ "cltID" => intval($_POST["cltID"]) ]
			    );
				$callback_success = true;
				
				if ($filas_afectadas > 0) {
					$callback_response = "Cliente actualizado.";
				}
				else {
					$callback_response = "La operación fue exitosa, aunque el cliente no se actualizó.";
				}
			}
		...
		$callback_response = "No se recibió el parámetro 'cltID' para actualizar un cliente.";
	}
}
