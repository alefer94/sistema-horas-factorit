...
if ($ajaxUsrID === -1){
	...
	if (isset($_POST["uslID"]) && is_numeric($_POST["uslID"])){
		if (sizeof($_POST) > 1) {
			...
			try {
				$pdo = new InterfazPDO();
				$filas_afectadas = PDOSkillUsuarios::update(
					$pdo, 
					$_POST, 
				  [ "uslID" => intval($_POST["uslID"]) ]
			    );
				$callback_success = true;
				
				if ($filas_afectadas > 0) {
					$callback_response = "Skill - Nivel actualizado.";
				}
				else {
					$callback_response = "La operación fue exitosa, aunque el Skill - Nivel no se actualizó.";
				}
			}
		...
		$callback_response = "No se recibió el parámetro 'uslID' para actualizar un cliente.";
	}
}
