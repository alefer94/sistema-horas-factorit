...
if ($ajaxUsrID === -1){ 
	...
	if (isset($_POST["prjID"]) && is_numeric($_POST["prjID"])){
		...
		$prjInterno = !empty($_POST["prjInterno"])?$_POST["prjInterno"] : null;
		...
		try { 
			...
			if ($filas_afectadas > 0){
				$callback_response = "El Costo Interno  del proyecto fue actualizado.<br/>";
			}
			else {
				$callback_response = "La operación fue exitosa, aunque el Costo Interno no fue actualizado.";
			}
		}
