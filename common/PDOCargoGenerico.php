 ...
 * Cliente: FactorIT
 * Archivo fuente: common/PDOCargoContractual.php
 * Autor: Cristian Lobos
 *
 * Clase de abstracción de consultas PDO para operaciones con la tabla cargo_contractual.
 */
require_once __DIR__.'/InterfazPDO.php';
require_once __DIR__.'/pdo_filters_array_to_string.php';
setlocale(LC_TIME, "");

abstract class PDOCargoGenerico{


        private const COLUMNAS = ["cgcID","cgcNombre"];
        ...
        public function select($pdo, $limit = 150, $page = 1, $filtros = []) {
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" && 
                ...
                $order_by_clause = "ORDER BY cgcID ";
        ...
        public function insert($pdo = null, $datos = []){
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
                ...
                if (sizeof($columnas_invalidas) === 0) {	
                    return $pdo->insert("cargo_generico", $datos);
                }
                else {				
        ...
        public function update($pdo = null, $datos = [], $filtros = []) {
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
                ...
                if (isset($datos["cgcID"])) { unset($datos["cgcID"]); }
                ...
                if (sizeof($columnas_datos_invalidas) === 0 && sizeof($columnas_filtros_invalidas) === 0) {	
                    return $pdo->update("cargo_generico", $datos, $filtros);
                }
                else {
                    $mensaje_excepcion = "";
        ...
        public function remove($pdo = null, $cgcID = 0){
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
                is_numeric($cgcID) && $cgcID > 0 ){
