...
abstract class PDOClientes{

	private const COLUMNAS = ["cltID", "cltCodigo", "cltNombre","empID","indID"];
		...
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" && 
			...
			$query = 
				"SELECT 
					...
					empID, 
					indID,
					(SELECT empNombre FROM empresas E WHERE E.empID = C.empID) AS 'empNombre',
					(SELECT indNombre FROM industrias I WHERE I.indID = C.indID) AS 'indNombre'
				FROM clientes C ";
			
			$condition_clause = pdo_filters_array_to_string($filtros);

			$order_by_clause = "ORDER BY cltID ";
    ...
    public function insert($pdo = null, $datos = []){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (sizeof($columnas_invalidas) === 0) {	
				return $pdo->insert("clientes", $datos);
			}
			else {				
    ...
    public function update($pdo = null, $datos = [], $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (isset($datos["cltID"])) { unset($datos["cltID"]); }
			...
			if (sizeof($columnas_datos_invalidas) === 0 && sizeof($columnas_filtros_invalidas) === 0) {	
				return $pdo->update("clientes", $datos, $filtros);
			}
			else {
				$mensaje_excepcion = "";
    ...
    public function remove($pdo = null, $cltID = 0){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			is_numeric($cltID) && $cltID > 0 ){
	...
	public function select_industrias($pdo = null){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO"){
			return $pdo->query(
				"SELECT 
					indID, 
