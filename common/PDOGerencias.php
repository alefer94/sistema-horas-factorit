 ...
 * Autor: Diego Garcia
 *
 * Clase de abstracción de consultas PDO para operaciones con la tabla gerencias.
 */
require_once __DIR__.'/InterfazPDO.php';
require_once __DIR__.'/pdo_filters_array_to_string.php';
setlocale(LC_TIME, "");

abstract class PDOGerencias{

	private const COLUMNAS = ["gerID", "gerCodigo", "gerNombre"];
    ...
    public function select($pdo, $limit = 150, $page = 1, $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" && 
			...
			$order_by_clause = "ORDER BY gerID ";
    ...
    public function insert($pdo = null, $datos = []){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (sizeof($columnas_invalidas) === 0) {	
				return $pdo->insert("gerencias", $datos);
			}
			else {				
    ...
    public function update($pdo = null, $datos = [], $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (isset($datos["gerID"])) { unset($datos["gerID"]); }
			...
			if (sizeof($columnas_datos_invalidas) === 0 && sizeof($columnas_filtros_invalidas) === 0) {	
				return $pdo->update("gerencias", $datos, $filtros);
			}
			else {
				$mensaje_excepcion = "";
    ...
    public function remove($pdo = null, $gerID = 0){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			is_numeric($gerID) && $gerID > 0 ){
