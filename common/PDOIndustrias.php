 ...
 * Cliente: FactorIT
 * Archivo fuente: common/PDOIndustrias.php
 * Autor: Cristian Lobos
 *
 * Clase de abstracción de consultas PDO para operaciones con la tabla industrias
 */
require_once __DIR__.'/InterfazPDO.php';
require_once __DIR__.'/pdo_filters_array_to_string.php';
setlocale(LC_TIME, "");

abstract class PDOIndustrias{

	private const COLUMNAS = ["indID","indNombre"];
    ...
    public function select($pdo, $limit = 150, $page = 1, $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" && 
			...
			$order_by_clause = "ORDER BY indID ";
    ...
    public function insert($pdo = null, $datos = []){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (sizeof($columnas_invalidas) === 0) {	
				return $pdo->insert("industrias", $datos);
			}
			else {				
    ...
    public function update($pdo = null, $datos = [], $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (isset($datos["indID"])) { unset($datos["indID"]); }
			...
			if (sizeof($columnas_datos_invalidas) === 0 && sizeof($columnas_filtros_invalidas) === 0) {	
				return $pdo->update("industrias", $datos, $filtros);
			}
			else {
				$mensaje_excepcion = "";
    ...
    public function remove($pdo = null, $indID = 0){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			is_numeric($indID) && $indID > 0 ){
