 ...
 * Cliente: FactorIT
 * Archivo fuente: common/PDOGerencias.php
 * Autor: Diego Garcia
 *
 * Clase de abstracción de consultas PDO para operaciones con la tabla subgerencias.
 */
require_once __DIR__.'/InterfazPDO.php';
require_once __DIR__.'/pdo_filters_array_to_string.php';
setlocale(LC_TIME, "");

abstract class PDOSubGerencias{

	private const COLUMNAS = ["sgrID", "sgrCodigo", "sgrNombre"];
    ...
    public function select($pdo, $limit = 150, $page = 1, $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" && 
			...
			$order_by_clause = "ORDER BY sgrID ";
    ...
    public function insert($pdo = null, $datos = []){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (sizeof($columnas_invalidas) === 0) {	
				return $pdo->insert("subgerencias", $datos);
			}
			else {				
    ...
    public function update($pdo = null, $datos = [], $filtros = []) {
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			...
			if (isset($datos["sgrID"])) { unset($datos["sgrID"]); }
			...
			if (sizeof($columnas_datos_invalidas) === 0 && sizeof($columnas_filtros_invalidas) === 0) {	
				return $pdo->update("subgerencias", $datos, $filtros);
			}
			else {
				$mensaje_excepcion = "";
    ...
    public function remove($pdo = null, $sgrID = 0){
		if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
			is_numeric($sgrID) && $sgrID > 0 ){
