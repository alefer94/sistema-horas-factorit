 ...
 * Cliente: FactorIT
 * Archivo fuente: common/PDOVicepresidencias.php
 * Autor: Cristian Lobos
 *
 * Clase de abstracción de consultas PDO para operaciones con la tabla vicepresidencias.
 */
require_once __DIR__.'/InterfazPDO.php';
require_once __DIR__.'/pdo_filters_array_to_string.php';
setlocale(LC_TIME, "");

abstract class PDOVicepresidencias{


        private const COLUMNAS = ["vprID", "vprCodigo", "vprNombre"];
        ...
        public function select($pdo, $limit = 150, $page = 1, $filtros = []) {
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" && 
                ...
                $order_by_clause = "ORDER BY vprID ";
        ...
        public function insert($pdo = null, $datos = []){
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
                ...
                if (sizeof($columnas_invalidas) === 0) {	
                    return $pdo->insert("vicepresidencias", $datos);
                }
                else {				
        ...
        public function update($pdo = null, $datos = [], $filtros = []) {
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
                ...
                if (isset($datos["vprID"])) { unset($datos["vprID"]); }
                ...
                if (sizeof($columnas_datos_invalidas) === 0 && sizeof($columnas_filtros_invalidas) === 0) {	
                    return $pdo->update("vicepresidencias", $datos, $filtros);
                }
                else {
                    $mensaje_excepcion = "";
        ...
        public function remove($pdo = null, $vprID = 0){
            if (is_object($pdo) && get_class($pdo)==="InterfazPDO" &&
                is_numeric($vprID) && $vprID > 0 ){
