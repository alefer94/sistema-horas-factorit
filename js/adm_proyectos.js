system.projects = system.projects || {
	...
	alFiltrar: function (event, columna_filtro) {
	  ...
	  var promise = $.get(
		...
		function (data) {
		  system.modal(
			'<h4 class="modal-title bold">Filtrar por ' +
			  columna_filtro +
			  "</h4>",
			data,
			"complete"
	...
	alConfirmarFiltro: function (event) {
	  ...
	  var promise = $.get(
		...
		ajaxData,
		function (data) {
		  $("#tbl-mantenedor thead span.btn-filtrar")
			.text("(Filtrado)")
	...
	alEditar: function (event, hitos,hrsProyecto) {
	  ...
	  $tr.html(newRow(hitos, hrsProyecto));
	  $tr.data("sid", prjID);
	  ...
	  $tr
		.find("select.slt-estado-facturacion-pry")
		.val(estado_facturacion_pry)
		.select2();
	...
	alGuardar: function (event, hitos, hrsProyecto) {
	  ...
	  var $slt_estado_facturacion_pry = $tr.find(
		"select.slt-estado-facturacion-pry"
	  );
	  ...
	  if (
		...
		if ($ipt_codigo.val().length != 7) {
		  system.modalInformacion(
			"Alerta ",
			"El codigo de proyecto debe tener 7 caracteres."
		  );
		  ...
		  if (codigo_iniciales == "COP" || codigo_iniciales == "FIT") {
			...
			if (
			  codigo_identificador_2 != null &&
			  codigo_identificador_2 != undefined &&
			  !isNaN(codigo_identificador_2) &&
			  codigo_identificador_2 >= 0 &&
			  codigo_identificador_2 <= 9999
			) {
			  ...
			  var promise = $.post(
				...
				function (data) {
				  ...
				  if (data[0]) {
					...
					var $opt_estado_facturacion_pry =
					  $slt_estado_facturacion_pry.find("option:selected");
					var $opt_estado_pry = $slt_estado_pry.find("option:selected");
					var $opt_centro_cos = $slt_centro_cos.find("option:selected");
					var $opt_vicepresidencias =
					  $slt_vicepresidencias.find("option:selected");
					var $opt_gerencias = $slt_gerencias.find("option:selected");
					var $opt_subgerencias =
					  $slt_subgerencias.find("option:selected");
					...
					$tr
					  .find("td:eq(2)")
					  .removeClass("no-padding no-border")
					  .data("sid", $opt_cliente.val())
					  .html($opt_cliente.text());
					$tr
					  .find("td:eq(3)")
					  .removeClass("no-padding no-border")
					  .data("sid", $opt_jefe.val())
					  .html($opt_jefe.text());
					$tr
					  .find("td:eq(4)")
					  .removeClass("no-padding no-border")
					  .data("sid", $opt_comercial.val())
					  .html($opt_comercial.text());
					$tr
					  .find("td:eq(5)")
					  .removeClass("no-padding no-border")
					  .data("sid", $opt_estado_facturacion_pry.val())
					  .html($opt_estado_facturacion_pry.text());
					$tr
					  .find("td:eq(6)")
					  .removeClass("no-padding no-border")
					  .data("sid", $opt_estado_pry.val())
					  .html($opt_estado_pry.text());
					$tr
					  .find("td:eq(7)")
					  .removeClass("no-padding no-border")
					  .data("sid", $opt_centro_cos.val())
					  .html($opt_centro_cos.text());
					$tr
					  .find("td:eq(8)")
					  .removeClass("no-padding")
					  .html($ipt_codigo_padre.val());
					$tr
					  .find("td:eq(9)")
					  .removeClass("no-padding")
					  .addClass("text-center b-r b-grey")
					  .html($ipt_costo_venta.val());
					$tr
					  .find("td:eq(10)")
					  .removeClass("no-padding")
					  .addClass("text-center b-r b-grey")
					  .html($ipt_margen_esperado.val());
					$tr
					  .find("td:eq(11)")
					  .removeClass("no-padding")
					  .addClass("text-center b-r b-grey")
					  .data("sid", $opt_vicepresidencias.val())
					  .html($opt_vicepresidencias.text());
					$tr
					  .find("td:eq(12)")
					  .removeClass("no-padding")
					  .addClass("text-center b-r b-grey")
					  .data("sid", $opt_gerencias.val())
					  .html($opt_gerencias.text());
					$tr
					  .find("td:eq(13)")
					  .removeClass("no-padding")
					  .addClass("text-center b-r b-grey")
					  .data("sid", $opt_subgerencias.val())
					  .html($opt_subgerencias.text());
  
					//se mandan los hitos y horas a la función que costruye los botones
					$td_buttons.html(updatedTd(hitos,hrsProyecto));
			  ...
			  system.modalInformacion(
				"Alerta ",
				'El Formato de codigo de proyecto debe ser el siguiente "COP9999" o "FIT9999" , error en la parte final del codigo.'
			  );
			  $this.prop("disabled", false);
			}
		  } else {
			system.modalInformacion(
			  "Alerta ",
			  'El Formato de codigo de proyecto debe ser el siguiente "COP9999" o "FIT9999" , error en la parte inicial del codigo.'
			);
			$this.prop("disabled", false);
		  }
		}
	...
	alBorrar: function (event, hitosFacturados, hrsProyecto) {
	  var $tr = $(event.target).closest("tr");
	  var prjID = Number($tr.data("sid"));
	  var existe = !isNaN(prjID);
  
	  /* Realizamos condicionales para ver si el proyecto tiene hitos facturados o horas, de ser asi
	  no se elimina y devolvemos el respectivo msj */
	  if (hitosFacturados > 0 ) {
		system.modalInformacion( "No se puede eliminar el proyecto",
		"El proyecto contiene hitos facturados.<br/>No puede realizar la eliminación.",
		"danger")
	  }
	  else if(hrsProyecto > 0) {
		system.modalInformacion( "No se puede eliminar el proyecto",
		"El proyecto contiene horas registradas.<br/>No puede realizar la eliminación.",
		"danger")
	  } else {
		if (!existe) {
		  $tr.remove();
		  ...
		  system.modalConfirmacion(
			...
			"danger",
			function () {
			  system.projects.alConfirmarEliminacion($tr);
			}
		  );
		}
	  }
