system.project_details.hitos = system.project_details.hitos || {
	...
	alGuardar: function (event) {
	  console.log("funcion al guardar");
	  ...
	  $filas_hitos.each(function (i) {
		...
		var $ipt_fch_fact = $(this).find("input.ipt-hito_fch_fact");
		  console.log(!$ipt_fch_trm[0].checkValidity())
		if (
	  ...
	  if(hitosIncompletos >0){
		  system.modalInformacion(
			  "Alerta ",
			  "Debe rellenar todos los campos para poder guardar"
			);
	  ...
	  //     "Confirmar acción",
	  //     "Hay hitos con información incompleta o incorrecta, y serán ignorados. ¿Desea continuar?",
	  //     "complete",
	  //     function () {
	  //       system.project_details.hitos.alConfirmar(hitos);
	  //     }
	  //   );
	  // }
